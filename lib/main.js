...
const runFormatProjects = (projectItems, label) => {
  ...
  const busyMessage = busySignal
    ? busySignal.reportBusy(`prettier: Formatting ${targetCount} ${label}...`)
    : null;
  ...
  task.on("prettier:format-progress", ({ projectPath, current, total }) => {
    if (busyMessage && shouldUpdateProgressTitle({ current, total })) {
      const action = current === total ? "Finished" : "Formatting";
      busyMessage.setTitle(`prettier: ${action} ${path.basename(projectPath)} (${total})`);
    }
  });
...
const consumeBusySignalReporter = (service) => {
  busySignal = service;
};
