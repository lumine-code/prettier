...
const activate = () => {
  ...
  subscriptions.add(
    atom.commands.add("atom-workspace", "prettier:format", (event) => {
      ...
      const clicked = event?.target?.closest?.("atom-text-editor:not([mini])")?.getModel?.();
      const editor = clicked ?? atom.workspace.getActiveTextEditor();
